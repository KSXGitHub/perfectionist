...
Partially implemented in `src/rules/needless_borrowed_parameter.rs`. The
...
- Exemptions: trait-declaration and trait-`impl` methods, parameters
  ...
  `ui/needless_borrowed_parameter_proc_macro.rs`).
...
- `clippy::needless_pass_by_value` covers the *opposite* direction
  ...
  `needless_borrowed_parameter` for full coverage of the pacquet
