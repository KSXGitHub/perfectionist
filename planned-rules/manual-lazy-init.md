...
disjoint config, and no shared diagnostic* — lock-vs-cell has none of
those, so they are one rule. The
...
the diagnostic span here is the whole declaration, which is wider
than an identifier, so the built-in `report_in_external_macro: false`
filter likely suffices — but
