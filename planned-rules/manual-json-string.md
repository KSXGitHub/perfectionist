...
- **JSON detection — format mode.** For a macro invocation that
  ...
  [`long-unsplittable-format-macro`](./long-unsplittable-format-macro.md) leans on the same
  ...
  and [`long-unsplittable-format-macro`](./long-unsplittable-format-macro.md)) and emit a
...
- [`long-unsplittable-format-macro`](./long-unsplittable-format-macro.md) — same
  suppression direction: when a `format!` invocation parses as
  a structurally interesting JSON document, `long_unsplittable_format_macro`
