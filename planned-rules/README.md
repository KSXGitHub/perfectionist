...
- [`pipe-style.md`](./pipe-style.md) — bidirectional pipe-trait
  ...
  `chain.pipe(f)`), flags `f(&mut chain)` wrapping a borrow of one
  (suggests `chain.pipe_mut(f)`), and flags a unary call heading a
  chain that pipes (suggests `x.pipe(g)`). All four checks default
  to enforce.
