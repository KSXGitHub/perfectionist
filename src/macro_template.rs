...
//!   `unsplit_print_macro` uses it because it may only ever touch a genuine
...
//!
//! Both skip raw strings (`r"..."`): `unsplit_print_macro` would mis-fold
...
fn cooked_str_literal_span(tree: &TokenTree) -> Option<Span> {
    ...
    // `unsplit_print_macro` nor the escape-elimination scan in
...
/// `code { "..." }`, which a top-level-only scan (as `unsplit_print_macro`
