...
declare_tool_lint! {
    ...
    /// }
    /// ```
    pub perfectionist::NEEDLESS_BORROWED_PARAMETER,
    Warn,
...
const CONFIG_KEY: &str = "perfectionist::needless_borrowed_parameter";
...
pub struct NeedlessBorrowedParameter {
    conversion_methods: BTreeSet<Symbol>,
}

impl NeedlessBorrowedParameter {
    fn new() -> Self {
...
impl_lint_pass!(NeedlessBorrowedParameter => [NEEDLESS_BORROWED_PARAMETER]);
...
pub fn register_lint(lint_store: &mut LintStore) {
    lint_store.register_lints(&[NEEDLESS_BORROWED_PARAMETER]);
}

/// Install this rule's late pass.
pub fn register_pass(lint_store: &mut LintStore) {
    if let DefaultState::Inactive = resolved_state("needless_borrowed_parameter", DEFAULT_STATE) {
        return;
    }
    lint_store.register_late_pass(|_| Box::new(NeedlessBorrowedParameter::new()));
}

impl<'tcx> LateLintPass<'tcx> for NeedlessBorrowedParameter {
    fn check_fn(
        &mut self,
        cx: &LateContext<'tcx>,
        kind: FnKind<'tcx>,
