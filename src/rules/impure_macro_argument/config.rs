//! Configuration for `impure_macro_argument`. Owns the [`Mode`] enum,
//! the user-facing [`Config`] shape, the curated built-in deny / allow
//! sets, and the in-memory [`ImpureMacroArgument`] state the early
//! pass holds.
//!
...
const CONFIG_KEY: &str = "perfectionist::impure_macro_argument";
...
pub(super) struct ImpureMacroArgument {
    mode: Mode,
