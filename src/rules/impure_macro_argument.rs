...
use config::ImpureMacroArgument;
use late::ImpureMacroArgumentLate;
...
declare_tool_lint! {
    ...
    /// ```
    pub perfectionist::IMPURE_MACRO_ARGUMENT,
    Warn,
    "macro invocation passes an impure expression that should be bound to a `let` first",
    report_in_external_macro: false
}

impl_lint_pass!(ImpureMacroArgument => [IMPURE_MACRO_ARGUMENT]);
impl_lint_pass!(ImpureMacroArgumentLate => [IMPURE_MACRO_ARGUMENT]);

pub fn register_lint(lint_store: &mut LintStore) {
    lint_store.register_lints(&[IMPURE_MACRO_ARGUMENT]);
}

pub fn register_pass(lint_store: &mut LintStore) {
    if let DefaultState::Inactive = resolved_state("impure_macro_argument", DefaultState::Active) {
        return;
    }
    ...
    lint_store.register_pre_expansion_pass(|| Box::new(ImpureMacroArgument::new()));
    lint_store.register_late_pass(|_| Box::new(ImpureMacroArgumentLate));
}
...
impl EarlyLintPass for ImpureMacroArgument {
