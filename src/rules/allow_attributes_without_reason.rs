...
declare_tool_lint! {
    ...
    /// fn build_fetcher(/* ... */) {}
    /// ```
    pub perfectionist::ALLOW_ATTRIBUTES_WITHOUT_REASON,
    Warn,
    r#"`#[allow]` / `#[expect]` attribute lacks an explanatory `reason = "..."` field"#,
    report_in_external_macro: false
}

const CONFIG_KEY: &str = "perfectionist::allow_attributes_without_reason";
...
pub struct AllowAttributesWithoutReason {
    exempt_lints: BTreeSet<String>,
    min_reason_length: NonZeroUsize,
}

impl AllowAttributesWithoutReason {
    fn new() -> Self {
...
impl Register for rule::AllowAttributesWithoutReason {
    ...
    fn register_pass(lint_store: &mut LintStore) {
        lint_store
            .register_early_lint_pass(Box::new(|| Box::new(AllowAttributesWithoutReason::new())));
    }
}
