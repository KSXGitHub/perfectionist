...
/// `separate_reexports` reserves the leading rank `0` for re-exports and
/// shifts every other rank up by one, so a run that mixes re-exports with
/// private imports carries one blank line between the two — even under
/// `single_block`.
