//! Configuration for `impure_macro_arguments`. Owns the [`Mode`] enum,
//! the user-facing [`Config`] shape, the curated built-in deny / allow
//! sets, and the in-memory [`ImpureMacroArguments`] state the early
//! pass holds.
//!
...
const CONFIG_KEY: &str = "perfectionist::impure_macro_arguments";
...
pub(super) struct ImpureMacroArguments {
    mode: Mode,
