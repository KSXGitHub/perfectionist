...
declare_tool_lint! {
    ...
    /// }
    /// ```
    pub perfectionist::CLAP_HELP_NO_MARKDOWN,
    Warn,
...
const CONFIG_KEY: &str = "perfectionist::clap_help_no_markdown";

pub struct ClapHelpNoMarkdown {
    config: ResolvedConfig,
}

impl ClapHelpNoMarkdown {
    fn new() -> Self {
...
impl_lint_pass!(ClapHelpNoMarkdown => [CLAP_HELP_NO_MARKDOWN]);

pub fn register_lint(lint_store: &mut LintStore) {
    lint_store.register_lints(&[CLAP_HELP_NO_MARKDOWN]);
}

pub fn register_pass(lint_store: &mut LintStore) {
    if let DefaultState::Inactive = resolved_state("clap_help_no_markdown", DEFAULT_STATE) {
        return;
    }
...
impl<'tcx> LateLintPass<'tcx> for ClapHelpNoMarkdown {
