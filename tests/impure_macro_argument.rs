//! UI tests for `impure_macro_argument`'s configuration knobs. The
//! default-config sweep lives in `ui/impure_macro_argument.rs` and is
//! picked up by `tests/ui.rs`; these tests each point at their own
//! one-fixture directory under `ui-toml/impure_macro_argument/` and
...
const LINT_NAME: &str = "perfectionist::impure_macro_argument";
...
fn deny_only_silently_accepts_uncatalogued_macros() {
    run(
        "ui-toml/impure_macro_argument/mode_deny_only",
        RuleConfig {
            mode: Some("deny_only"),
            ..Default::default()
        },
...
fn blanket_flags_allow_set_macros_too() {
    run(
        "ui-toml/impure_macro_argument/mode_blanket",
        RuleConfig {
            mode: Some("blanket"),
            ..Default::default()
        },
...
fn deny_extra_adds_a_qualified_macro_to_the_deny_set() {
    run(
        "ui-toml/impure_macro_argument/deny_extra",
        RuleConfig {
...
fn allow_extra_silences_an_uncatalogued_macro() {
    run(
        "ui-toml/impure_macro_argument/allow_extra",
        RuleConfig {
...
fn extra_pure_methods_adds_a_pure_getter_to_the_postfix_set() {
    run(
        "ui-toml/impure_macro_argument/extra_pure_methods",
        RuleConfig {
...
fn ignore_pure_methods_drops_a_built_in_pure_getter() {
    run(
        "ui-toml/impure_macro_argument/ignore_pure_methods",
        RuleConfig {
...
fn extra_pure_macros_adds_a_compile_time_macro_to_the_atom_set() {
    run(
        "ui-toml/impure_macro_argument/extra_pure_macros",
        RuleConfig {
...
fn ignore_pure_macros_drops_a_built_in_compile_time_macro() {
    run(
        "ui-toml/impure_macro_argument/ignore_pure_macros",
        RuleConfig {
...
fn ignore_suppresses_a_built_in_deny_set_macro() {
    run(
        "ui-toml/impure_macro_argument/ignore",
        RuleConfig {
...
fn ignore_wins_over_deny_extra_for_the_same_macro() {
    run(
        "ui-toml/impure_macro_argument/ignore_overrides_deny_extra",
        RuleConfig {
...
fn fn_level_expect_fulfils_against_a_violation() {
    run(
        "ui-toml/impure_macro_argument/expect_at_fn_scope",
        RuleConfig::default(),
    );
}
...
fn crate_level_expect_fulfils_against_a_violation() {
    run(
        "ui-toml/impure_macro_argument/expect_at_crate_root",
        RuleConfig::default(),
    );
}
...
fn module_level_expect_fulfils_for_item_position_macro() {
    run(
        "ui-toml/impure_macro_argument/expect_at_module_with_item_macro",
        RuleConfig::default(),
    );
}
