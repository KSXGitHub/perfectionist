...
//! of bug reported for `import_granularity_mismatch` in
...
//! different modules, so `import_granularity_mismatch`'s module style does not
//! also fire on them.
...
const LINT: &str = "perfectionist::import_grouping_mismatch";
...
#[allow(perfectionist::import_grouping_mismatch, reason = \"regression fixture\")]
mod separate;
";
    let separate = "\
use std::collections::BTreeMap;
