//! UI tests for `overlong_print_macro`'s configuration knobs. The
//! default-config sweep lives in `ui/overlong_print_macro.rs` and is
//! picked up by `tests/ui.rs`; these tests each point at their own
//! one-fixture directory under `ui-toml/overlong_print_macro/` and pass a
...
const LINT_NAME: &str = "perfectionist::overlong_print_macro";
...
fn narrow_width_flags_a_line_within_the_default_limit() {
    ...
    run(
        "ui-toml/overlong_print_macro/narrow_width",
        RuleConfig {
...
fn target_macros_replaces_the_built_in_list_wholesale() {
    ...
    run(
        "ui-toml/overlong_print_macro/custom_target_macros",
        RuleConfig {
