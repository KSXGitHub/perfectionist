...
const LINT: &str = "perfectionist::import_granularity_mismatch";
...
fn flags_split_in_separate_file_submodule() {
    ...
    let (_temp, stderr, success) = run_project_with_sources(
        ...
        &[
            (
                "src/lib.rs",
                include_str!("fixtures/import_granularity_mismatch_submodules/flags_lib.rs"),
            ),
...
fn respects_allow_on_separate_file_submodule() {
    ...
    let (_temp, stderr, success) = run_project_with_sources(
        ...
        &[
            (
                "src/lib.rs",
                include_str!("fixtures/import_granularity_mismatch_submodules/allowed_lib.rs"),
            ),
            ("src/separate.rs", separate),
        ],
    );
